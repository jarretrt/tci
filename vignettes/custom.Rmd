...
<!-- 
Exogenous phosphocreatine (phosphocreatine sodium)
  - Given to allow the body to produce ATP during myocardial injury (e.g., during pediatric myocarditis)
  - Administered via IV
  - Breaks down into metabolite creatine
  - Described with a 4 compartment model [@He2020]
  - Description of Pcr, cr, in @Balestrino2021
  
Ketamine
- Otto et al. 2021. Also useful example of external PK model validation.
- IV administration, takes effect immediately (relative to SSRIs), and lasts for weeks to months 
- Used to treat treatment-resistant depression
- Results in adverse effects at high doses
- S-Ketamine turns into norketamine, which is present in much higher concentrations at steady state
  - "administering S-ketamine orally is like administering a pro-drug of norketamine" [@Fanta2015]
- Ketamine and norketamine may produce effects in opposite directions (ideal for TCI) [@Olofson2012]


IV antibiotics used for sepsis
- Time dependent
- Important to get correct concentration and duration for safety of patient and prevention
of antimicrobial resistance
- Ideal for patient personalization

- "Prolonged duration of antimicrobial therapy is associated with increased resistance, higher risk of drug-associated adverse effects, and higher costs."
- "short treatments are safer than longer treatments, without affecting treatment success"
- "Optimized antibiotic dos ing regimens to ensure therapeutic levels of antimicrobials improve clinical results, minimize drug toxicity and drug-resistance, and reduce mortality in critically ill patients"


- TCI has been used for vancomycin [@Colin2018] and cefepime [@Jonckheere2020] 

Models/drugs commonly used with TCI:
(https://www.bjanaesthesia.org/article/S0007-0912(18)30120-X/fulltext)

1. Propofol: Marsh, Schnider, Eleveld
2. Remifenanyl: Minto, Eleveld, Kim

It is highly likely that TCI systems will be used to administer antibiotics in the future for personalized dosing in critically ill patients. [@Roberts2019]
...
When `pk_remif` is evaluated, it will return  
...
```{r}
dose_remi <- create_inf(times = 0, infrt = 60, duration = 20)
...
The required arguments for a custom algorithm are 1) a single numeric value specifying the target concentration, `Ct`, 2) a `pkmod` object created by `pkmod()`, 3) a single numeric value specifying the infusion duration, `dtm`, and 4) additional arguments that are passed to `update.pkmod` at the beginning of the algorithm. Argument (4) is used to update starting concentrations when the algorithm is iteratively applied in `apply_tci`. 
...
tci_plasma_lim <- function(Ct, pkmod, dtm = 1/6, maxrt = 1200,
  ...
  unit_inf <- create_inf(times = 0, infrt = pinf, duration = dtm)
...
More usefully, however, we can pass the algorithm to `apply_tci` through the `custom_alg` argument and use it to calculate infusion rates required to reach a series of targets.
...
plim_inf <- apply_tci(targets = tci_targets,
                      pkmod = mod3ecpt, 
...
eff_inf <- apply_tci(targets = tci_targets, pkmod = mod3ecpt, type = "effect")
```
