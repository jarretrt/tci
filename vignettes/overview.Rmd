...
Functions simulating open- and closed-loop control are implemented in `tci` and can be used to predict patient responses and observed measurements under model misspecification without or with feedback. These are implemented via the functions `simulate_olc` and `simulate_clc` for open- and closed-loop control, respectively. Both functions require two `pkmod` or `poppkmod` objects representing the prior model for a set of individuals and the "true" models that will be used to simulate observations. They additionally require targets and target times to be passed to the TCI algorithm and times at which observations should be made. Closed-loop control further requires a set of times at which the prior PK/PKPD model should be updated based on the  available measurements. 

### Open-loop control

To illustrate open-loop control, we simulate PK responses from a three-compartment model at times 1, 2, 4, 8, 12, 16, and 24 over a 24 hour period in which effect-site targeting is used and the target concentration is raised from 2 mg/L to 4 mg/L. 
...
Under Bayesian closed-loop control, a prior PK/PK-PD model is updated in response to observed data. In `tci` simulations, the observed data are generated by an underlying true model. 


**PK example - plasma targeting**

Below we illustrate the following scenario. A medication is administered to a patient via IV infusion once every 24 hours for three days. Each infusion is administered over the course of an hour at a rate calculated to reach a target plasma concentration of 3 mg/L.
