...
As seen in the above plot, continuing to target the effect-site results in substantial oscillations within the plasma concentration. This can be avoided by allowing the algorithm to switch to plasma targeting when the plasma and effect site concentrations are within bounds of the targets. @Jacobs1993 suggest switching when the plasma concentration is within 10\% of the target and the effect-site concentration is within 0.5\% of the target.

We introduce modification of the above TCI algorithms that implements this procedure.
...
We now define functions to generate hypothetical patient PK-PD data according to specified population PK or PK-PD models. While a number of commonly used models are provided, such as the Marsh and Schnider PK models or the Eleveld PK-PD model, it is also possible for the user to define custom PK models. Population models should be 

The user begins by creating a patient population object by passing a named list of patient covariate values to the function `gen_pop` along with the name of a population PK or PK-PD model. The function checks that all 

```{r, gen-patient-data}


gen_pop <- function(patient_cov, model)
